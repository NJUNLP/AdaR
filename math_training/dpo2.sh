...
for learning_rate in 1e-5 5e-6 1e-6 5e-7 1e-7; do
for total_bsz in 16 32; do

gpu_count=$(echo $CUDA_VISIBLE_DEVICES | tr ',' ' ' | wc -w)
accumula=$((total_bsz / batch_size / gpu_count))
datasets=("qwen7b_instruct-generate_dpo_pairs-qwen7b_instruct-generate_cot_with_code-qwen72b_instruct-generate_template_and_code-orca_10k_train-disturbed_1.0") 
num_train_epochs=1
...
output_dir=/home/nfs02/laizj/experiment/uncertainty_analysis/math_training/model/dpo-$model_name-$(IFS=,; echo "${datasets[*]}")-${num_train_epochs}-${learning_rate}
