...
for learning_rate in 1e-5 5e-6; do
    for total_bsz in 32 64; do
        no_shuffle=False
        gpu_count=$(echo $CUDA_VISIBLE_DEVICES | tr ',' ' ' | wc -w)
        accumula=$((total_bsz / batch_size / gpu_count))
        datasets=( 
        "qwen7b_instruct-generate_rft-qwen7b_instruct-generate_cot_with_code-qwen72b_instruct-generate_template_and_code-orca_10k_train-disturbed_1.0_16-4") 
        num_train_epochs=1
