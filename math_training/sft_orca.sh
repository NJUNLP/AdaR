...
for learning_rate in 1e-4 5e-5 1e-5 5e-6 1e-6; do
    for total_bsz in 16 32; do
        ...
        #     model_name=qwen7b
        #     output_dir=/home/nfs02/laizj/experiment/uncertainty_analysis/math_training/model/$model_name-${dataset}-${num_train_epochs}-${learning_rate}-${total_bsz}-no_shuffle
        ...
        #         --save_only_model \
        #         --report_to wandb \
        #         --run_name "$model_name-${dataset}-${num_train_epochs}-${learning_rate}-${total_bsz}"
            ...
            bash test_full.sh $model_name $output_dir
        done
    done
done
